...
public class ZiphySearchResultsController: NSObject {
    ...
    public func search(withSearchTerm searchTerm: String, _ completion:@escaping SuccessOrErrorCallback) -> CancelableTask? {
        ...
        self.paginationController?.fetchBlock = { [weak self] (offset) in
            ...
            if let strongSelf = self {
                ...
                return strongSelf.ziphyClient?.search(strongSelf.callbackQueue, term: searchTerm, resultsLimit: strongSelf.pageSize, offset: offset) { [weak self] (success, ziphs, error) -> Void in
                    if let strongSelf = self {
